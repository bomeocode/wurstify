...
<div id="manual-address-section" class="mb-3">
  ...
  <input
    ...
    name="address_manual"
    id="address_manual"
    placeholder="PLZ oder Adresse eingeben..."
    value="<?= old('address_manual', $vendor['address'] ?? '') ?>"
    <?= isset($vendor) ? 'readonly' : '' ?>>

  <button
    class="btn btn-outline-secondary w-100 mt-2"
    type="button"
    id="use-current-location"
    <?= isset($vendor) ? 'disabled' : '' ?>>
  ...
  <div class="form-text mt-1" id="location-status"></div>
...
<div class="mb-3">
  ...
  <input
    ...
    id="vendor_name"
    required
    placeholder="z.B. Der Wursti Grill"
    value="<?= old('vendor_name', $vendor['name'] ?? '') ?>"
    <?= isset($vendor) ? 'readonly' : '' // Feld sperren, wenn Vendor bekannt 
    ?>>
</div>
